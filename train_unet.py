...
from src.load_data import train_loader  # 导入 train_loader

# 初始化模型、损失函数和优化器
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = UNet(in_channels=3, num_classes=2).to(device)  # 输入3通道，输出2通道（二分类）
...
print("模型已保存到 unet_best_model.pth")
